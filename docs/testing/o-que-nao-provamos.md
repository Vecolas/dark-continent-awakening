...
| `AncoraEncostaNaMadeiraTest` | que a âncora de escalada nasce FORA da casca e a até 2,5 blocos dela, que a varanda alcança para dentro da madeira, e que o checkpoint é escrito antes da copa | **que dá para subir**. Alcance de pulo, espaço livre acima da varanda e se a rota entre dois checkpoints é escalável continuam sem régua. E o relevo do Overworld não entra na conta: com `y` fixo, um balcão pode nascer dentro de uma encosta |
