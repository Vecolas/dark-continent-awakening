...
| A fisgada do Master of the Swamp nunca aconteceu em jogo | `RegrasDeFisgada` é pura e testada, e a régua prova que fugir em linha reta arrebenta antes de o peixe cansar. Mas anzol, arranco, tensão medida em variação de distância, captura com a mão vazia e navegação aquática são **cálculo verificado, não comportamento medido** — não há gametest deste mob | gametest de pesca |
