...
| `runGameTestServer` | jogo carregado, perfil/runtime e 120 ticks de Aura pelo scheduler real; **146 cenários** (medido em `f5ae73b`), entre eles os inimigos: geometria do ponto fraco, fase de ataque voltando para IDLE, manada não se ferindo, e o agarrão do sapo montando, recusando desmontagem e soltando por tempo e por morte | rede física, desenho do cliente e performance não são provados pelo listener de captura; a vítima dos cenários de inimigo é um golem, não um jogador. E **que UMA execução verde signifique verde**: em 2026-09-21 mediu-se **1 reprovação em 5 execuções** (`1 required tests failed`), sem que o teste culpado fosse identificado — logo uma passagem única não distingue "passou" de "passou desta vez". Pior, **sem `--rerun-tasks` a tarefa devolve cache e "passa" sem subir servidor nenhum** |
