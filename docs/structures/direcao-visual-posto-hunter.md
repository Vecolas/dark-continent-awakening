# Direção visual — Posto de exploração Hunter

Este é o contrato de arte da issue #231. O posto precisa parecer uma equipe
experiente instalada em território hostil: modular, usado, competente e
reparável. A identidade vem da operação de campo, não de símbolos grandes.

Referência visual aprovada: `referencia-posto-hunter-v2.png`. Ela orienta
proporção, torre aberta, cercamento transparente, fachadas baixas, cabos,
lonas e iluminação técnica; não é um asset de gameplay.

## Leitura desejada

- madeira estrutural escura e funcional;
- metal pontual, rebites, barras e ferragens visíveis;
- lonas claras/cinzentas para cobertura e reparos;
- caixas, barris, mapas, mesas de pesquisa e amostras;
- iluminação prática, concentrada em acesso, pátio, pesquisa e vigia;
- torre de observação aberta, com plataformas e travamentos metálicos;
- portão logístico largo e cercamento baixo/transparente de iron bars;
- concreto branco como material estrutural, sem blocos de ferro maciços, e
  shroomlight como iluminação técnica;
- irregularidade de uso sem transformar o posto em ruína.

## Leitura proibida

- castelo ou fortaleza medieval;
- muralha maciça ou torre feudal;
- vila vanilla ou casa rural genérica;
- laboratório futurista;
- steampunk ornamental;
- excesso de brasões, bandeiras ou logos.
- fogueira central ou lanternas penduradas como linguagem dominante.

## Regra de revisão

Se a silhueta puder ser descrita como “castelo”, “forte medieval” ou “vila”,
o blockout reprova mesmo que esteja tecnicamente correto. O posto deve ser
reconhecido por torre de observação leve + pátio logístico + anexos de
pesquisa/abastecimento.

## Paleta de primeira passagem

Usar blocos vanilla como material de construção, mas em composição de posto de
campo: madeira escura, pedra apenas como fundação/contrapeso, ferragens e
tecido. Blocos customizados ficam para uma etapa posterior e não podem ser
necessários para a estrutura existir.
