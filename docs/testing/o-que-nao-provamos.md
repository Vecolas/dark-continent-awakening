...
| A distribuição por região **não tem régua na tela** | o renderer JÁ multiplica por região — `AuraPlayerRenderLayer` faz isso no passe de shell e nos filamentos, e a linha anterior desta tabela, que dizia o contrário, estava obsoleta desde o AV1 e foi corrigida aqui. O que falta é o outro lado do #175: o overlay de dev não mostra os seis fatores, então o multiplicador é um número que ninguém consegue observar | #175, quando o overlay ganhar os seis campos |
