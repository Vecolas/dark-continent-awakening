...
| `runGameTestServer` | jogo carregado, perfil/runtime e 120 ticks de Aura pelo scheduler real; **137 cenários**, entre eles os inimigos: catálogo EN1 carregado do datapack, fluxo end-to-end do DummyEnemy, geometria do ponto fraco, stagger do DummyEnemy, carga do Great Stamp atordoada por parede, fase de ataque voltando para IDLE, manada não se ferindo, ninho da Spider Eagle, captura/morte separadas do Master of the Swamp e julgamento/recompensa da Kiriko | rede física, desenho do cliente e performance não são provados pelo listener de captura; a vítima dos cenários de combate é um golem, não um jogador |
...
| O atordoamento na parede do Great Stamp não tem gametest | `GreatStampGameTest.aParedeAtordoaDuranteACarga` ergue uma parede durante o windup e observa o estado `STAGGERED`; colisões em terreno gerado continuam fora da prova | QA manual dos inimigos |
...
| **A Spider Eagle nunca foi vista nascer num mundo gerado** | o spawn da entidade em GameTest cria o ninho automaticamente, e o bloco/ovo podem ser obtidos sem matar a mãe com saque server-side; nenhum mundo gerado foi usado para confirmar densidade e posicionamento natural | playtest de canyon |
...
| O peixe nunca NADOU até a isca | `MasterOfTheSwampGameTest` prova em jogo a bocada, a linha arrebentando com quem foge, o cansaço por tempo e a captura com a mão vazia; a navegação aquática até uma isca em movimento continua sem cenário | gametest de navegação |
| O julgamento do Kiriko nunca rodou em jogo | `KirikoGameTest` prova nascer disfarçado, aprovação após 200 ticks parado, registro no bestiário, entrega da nota própria e fallback com inventário cheio. A revelação visual e a legibilidade do julgamento continuam sendo QA de cliente | QA manual dos inimigos |
...
| A captura e a morte a pancada dão o mesmo loot | a captura do Master of the Swamp registra `captured_by_fishing`, entrega `SWAMP_FIELD_NOTE` e usa o caminho de recolhimento; a morte continua usando exclusivamente a loot table da entidade | QA manual da economia |
| Sons continuam bloqueados por ferramenta | nenhum dos oito mobs emite som; a máquina não tem `ffmpeg` nem encoder Vorbis, e o Minecraft só toca `.ogg`. É bloqueio de ambiente, não de escopo — e enquanto durar, nenhum mob fecha a ficha do §14 | quando houver encoder |
...
GameTest e por dois clientes reais. Os testes de GameTest passam; o listener do
