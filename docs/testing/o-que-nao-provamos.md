...
| **Nenhum portao liga as DUAS tabelas de tecnica -> modo visual** | `ModoVisualDeTecnica` decide o que o proprio jogador ve; `EstadoVisualDeTerceiro` decide o que os outros veem. Elas ja discordam hoje -- Ken desenha para terceiros e nao desenha para si --, e nada reprova por isso. `VfxDeAuraLigadoTest` cobre Ten, Ren, Zetsu, vazio e `null`; nao cobre Ken, nem Gyo, nem Ko, nem Shu. Uma tecnica registrada que esqueca uma das duas tabelas entra em producao sem erro nenhum | quando o defeito do Ken for consertado: o portao nasce junto, e nao depois |
