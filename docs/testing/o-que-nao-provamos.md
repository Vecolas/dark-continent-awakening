...
| A ausência só é medida onde alguém pensou em medi-la | `CorpoDeTodoInimigoTest` nasceu porque dezesseis criaturas existiram registradas e **sem corpo**, com todos os portões verdes — cada régua media a coluna dela e nenhuma media o vazio. O portão novo cobre geo, animação, textura e renderer. Nada garante que a próxima coluna esquecida seja notada antes de aparecer em jogo | disciplina, não portão |
...
| A fundação EN1 nunca rodou com o jogo de pé | percepção com orçamento, stagger, `AttackController` ligado à entidade, perfis de spawn e o `EnemyRuntime` têm 60+ testes puros e **onze GameTests escritos**; nenhum GameTest foi EXECUTADO (`runGameTestServer` não rodou nesta trilha). Os dois defeitos que os cenários acharam — a caixa de golpe apontando para trás e a fase presa depois da morte — vieram da LEITURA que escrevê-los forçou, não de um verde | gate do EN1 (#139) |
...
| As sete criaturas de Greed Island estão registradas e **sem corpo** | EntityType, atributos, loot, tradução, perfil, card e renderer existem; geo, animação e textura de algumas ainda não. Em jogo elas são invisíveis, e nenhum portão reprova invisibilidade | quando os sete geradores de arte rodarem |
| Nenhuma criatura de Greed Island nasce em lugar nenhum | a dimensão da ilha não existe como datapack, e o controlador de encontro não tem spawner concreto. `GreedIslandRegion` já PROÍBE tudo que não seja a ilha — proibir primeiro, abrir depois | quando a dimensão existir |
| Não existe entidade de Chimera Ant | identidade, molde, orçamento de rastreamento, colônia e Nen tático são domínio puro, com 47 testes. Nenhuma formiga foi criada, nenhuma colônia foi salva em disco, e nenhuma intenção de Nen chegou a ativar uma técnica | EN8/EN9 com entidade |
...
| Os **143 GameTests nunca tinham rodado** | `runGameTestServer` crashava no primeiro lote desde o EN13 (#148): `EnemySoundEvents` registrava 120 sons num `DeferredRegister` já fechado, porque nada carregava a classe durante o registro — o único caminho até ela era `BaseHxHMob.getVoice()`, chamado quando um mob tenta falar. Corrigido nesta branch; agora **139 dos 143 passam** | corrigido |
| **Quatro GameTests de encontro falham**, e estavam mascarados pelo crash | `reconciliarcombichovivonaoduplica`, `matartudoconcluiedestravaarecompensa`, `oencontroarmadocolocacriaturanomundo`, `encontroabandonadofalhaemvezdeficarpreso`. Verificado na `main` com a mesma correção de som aplicada: **as mesmas quatro**, e nenhuma delas toca a trilha AV. É dívida do sistema de encontros, e não desta branch | issue própria, na lane de inimigos |
...
| O **passe de brilho nunca rodou numa GPU** | a cadeia inteira -- alvo com máscara de profundidade, downsample, desfoque separável, composite aditivo -- compila e tem teste da aritmética (pesos, raio, níveis, fallback). Nenhum pixel dela foi visto. O que pode falhar: tela preta em driver antigo, o `OutputStateShard` não restaurando o alvo principal, ou o blit de profundidade sendo recusado em silêncio | gate do AV5 (#198) |
