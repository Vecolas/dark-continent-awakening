...
| Sessão de julgamento do AV0 (2026-09-21) | que um humano olhou em `5a9182b` e a shell **não estoura** nas três luzes, **não é engolida** por skin de segunda camada, **não descola** ao correr/agachar/nadar, o LOD **não dá degrau** de 2 a 40 blocos, e **morte e troca de dimensão não deixam estado preso** — estas duas últimas não têm teste automatizado nenhum, e esta é a única prova que existe delas | **nada disso é revisável por terceiro: não há uma única imagem, e nunca haverá.** As catorze capturas foram APOSENTADAS em 2026-09-22 por decisão do dono do projeto, com o custo registrado em `capturas/AV0/DIVIDAS.md`: sem elas não há o que reabrir, não há linha de base para comparar o AV1, e uma regressão visual futura passa calada. A procedência FOI declarada e conferida contra o log — dedicado, dois jogadores (`Kurapika` e `Gon`), 51 minutos em `run/server/logs/latest.log` —, então o bloco de distância mediu o LOD contra outra entidade e vale. O log do dedicado FOI lido (ver a linha abaixo) |
| Sessão de dois jogadores (2026-09-21, `run/server`) | que o mod roda **51 minutos com DOIS jogadores** num servidor dedicado sem uma única `[ERROR]`, `[FATAL]`, `Exception` ou `Caused by`, e que o caminho C2S foi exercitado de verdade (`15 pedidos admitidos, 0 cortados antes da fila`) | **que não há classe client-only vazada.** Ela rodou no `runServer` (`--launchTarget forgeserverdev`), onde as classes de cliente **estão no classpath**: um `net.minecraft.client.*` alcançado pelo servidor encontra a classe e não lança nada — o log sai limpo pelo motivo errado. Cruzar produção COM jogador é a única forma, e ainda não foi feito |
| Varredura do log do dedicado (2026-09-21) | que o servidor DEDICADO DE VERDADE (`instancia/servidor`, NeoForge 21.1.250, JAR conferido por SHA-256) sobe até `Done`, carrega o mod (`Protocolo de rede v9`), cria `world_tree` e `greed_island` e desliga **sem uma única** `NoClassDefFoundError`, `ClassNotFoundException`, menção a `net.minecraft.client`/`nenfoundation.client`, `[ERROR]`, `[FATAL]` ou `Exception` | **que ninguém entrou.** O servidor subiu e desligou sozinho: vazamento alcançado no join, no caminho de payload ou no de render não dispara nesta varredura. E ela vale para a INSTÂNCIA, não para o `runServer` — no workspace de desenvolvimento as classes de cliente estão no classpath e um vazamento sairia do log limpo pelo motivo errado, que é o erro nº 10 do `CLAUDE.md` um nível acima |
| `MorteETrocaDeDimensaoTest` | que o estado guardado por **id de entidade** e esquecido quando o jogador morre ou troca de dimensao — os dois casos em que o cliente TROCA a entidade em vez de deslogar, e o id volta diferente. Prova o mecanismo (a poda por presenca), prova o controle (sem ela o estado ficaria preso) **e** prova a ligacao (que `AudioDeAura` chama `reterSomente` no tick), esta ultima verificada removendo a chamada do codigo real | **que a aura some na tela ao morrer.** Ele mede um `HashMap`, nao um quadro. E cobre UM detentor de estado por id — `DetectorDeAtivacaoDeTen`; um mapa novo por id de entidade em outro arquivo nasce sem regua, e a verificacao da ligacao le TEXTO de um unico arquivo |
| Sessao em PRODUCAO com jogador (2026-09-22) | que o dedicado de verdade (`instancia/servidor`, sem classes de cliente no classpath) aguenta **23 min com dois jogadores**, 37 pedidos C2S, 8 mortes por mobs do mod e as duas dimensoes proprias carregadas, com **zero** `NoClassDefFoundError`, `ClassNotFoundException`, mencao a `client` ou `[ERROR]` nos dois logs. E o unico cruzamento que fecha o criterio de classe client-only | **o save ao PARAR.** O servidor nao foi encerrado com `stop`: os logs terminam na saida do ultimo jogador, sem sequencia de shutdown. Um defeito de persistencia aparece ali, e ali nao foi medido nesta execucao — a de 21/09 fez o shutdown limpo, mas sem jogador. **"Producao + jogador + shutdown limpo" nunca aconteceu numa execucao so** |
| *(nenhuma regua)* — **sentido de Z dos membros pareados** | nada. **Nao ha portao, e a tentativa de escrever um falhou de propósito registrada aqui.** Foram tentadas duas regras: *"esquerda e direita tem sinais opostos"* — falsa, os bracos do boneco de treino PENDEM e tombam juntos com o corpo — e *"o extremo de Z do membro esquerdo e negativo"* — tambem falsa, uma asa que bate passa pelos DOIS sinais, e o `avian_commander` tem `wing_left` negativo no *strike* e positivo no *idle* | que as asas e os bracos abrem para o lado certo. A convencao (**no lado +X, quem abre para FORA e o Z NEGATIVO**) foi conferida por olho humano em 2026-09-22 e confirmada contra a vanilla (`HumanoidModel.setupAnim` soma ao `zRot` do braco direito e subtrai do esquerdo, afastando os dois do corpo). **So a tela mostra**, e a correcao aplicada a oito mobs esta declarada em `comum.animacao.MOBS_COM_PREMISSA_DE_Z_INVERTIDA`, mob a mob. **Perna e orelha nao foram viradas nem verificadas** |
| `ImpactoDeAuraTest` | que o ripple (#103) nasce na **queda de vida** (e nao na borda de `hurtTime`, que chega noutro tick); que um **soco de mao vazia acende de forma visivel**; que o realce **passa de 1.0** (sem o que ele era no-op em Ten, onde toda regiao ja vale 1.0); que ele **MULTIPLICA** e por isso **preserva a concentracao** -- quem esta em Ko continua em Ko durante o flash, o que somar achatava; que acende o **corpo inteiro por igual**, porque o cliente nao sabe onde o golpe acertou e ninguem finge saber; que em **Zetsu nao acende**; e que o pico tem **plato**, sem o que ele durava um quadro; que a forca e a fracao da vida MAXIMA perdida (e nao da atual, que faria o ultimo golpe de quem esta quase morto ser sempre o mais forte); que pancada ABSORVIDA e arranhao abaixo do piso **nao acendem**; que o realce nao apaga a distribuicao do servidor, decai, e nao aloca quando nao ha impacto | **que o ripple aparece na tela** -- e isso NAO tem regua, mas **foi verificado por olho humano em 2026-09-22** (*"no hit, na aura toda passa um pulso azul"*), depois de QUATRO correcoes. Os testes medem `float` e `HashMap`, nao pixel. E a LIGACAO continua sem regua: eles chamam `DetectorDeImpacto` direto, e que `NenFoundationClient` de fato chame `impactos.tick(...)` a cada tick e leia no funil de estado so a sessao mostra. **A linha de ripple no overlay F6 existe para esse buraco** -- ela nao prova, mas separa "nao dispara" de "dispara e nao se ve", que foi o que destravou o caso |
| Sessão de julgamento do AV1 (2026-09-22) | que com **partícula em ZERO** a aura ainda comunica Ten **e as outras técnicas** — o gate do #176 —, que a shell se lê nos sete ambientes (incluindo neve e Nether), que o modelo slim é o certo, que overlay de skin não a engole, que o ruído lê como **veios** com `escala_de_ruido` em 4.0, e que não há z-fighting no giro de 360° | **nada disso tem imagem** — a trilha dispensou captura em 2026-09-22, e a evidência é a folha `PERGUNTAS.md`, humana e datada. E **duas perguntas seguem sem resposta**: a série de distância 2b→40b (C1/C2) e o ripple #103 (E2), que **nunca foi visto em tela por ninguém**. O #176 não fecha sem elas |
| `ForcaDoBrilhoTest` | que a mascara de oclusao e copiada depois do **CUTOUT** (grama, folhas e flores ocluem o halo -- antes nao ocluiam, e ele atravessava exatamente esses blocos); que a escala do composite e CONSTANTE e recompoe o Ten em 0,04, a magnitude que passou no AV1; e que a forca do perfil entra UMA vez no halo: que o composite de tela inteira **nao** multiplica o peso por `maiorForca` (o valor de outra pessoa), que `maiorForca` segue decidindo o pulo e o raio, que a escrita do alvo segue aplicando a forca POR JOGADOR, e que Ren continua mais forte que Ten no dado | **que o halo esta certo na tela.** Ele le TEXTO da fonte, e a conta de verdade acontece na GPU. Uma terceira multiplicacao escrita de outro jeito, ou noutro arquivo, passa. **O defeito que ele tranca foi achado por olho humano em jogo, nao por regua** -- o build estava verde enquanto a aura de terceiros clareava 2,75x |
| Sessão de julgamento do AV2 (2026-09-22) | que os filamentos **nascem na superfície** (2b, 5b, 10b), **acompanham os membros** ao correr, agachar, nadar e atacar, nascem no lugar certo nos **dois modelos**, que dois quadros **congelados são idênticos**, e que com **partícula em zero** a aura COM filamentos ainda se lê | **nada disso tem imagem** — a trilha dispensou captura. E **os dois contadores do D2 não foram anotados**: filamentos vivos e chamadas de desenho são a linha de base que o AV8 compararia, e número não escrito não existe depois. A janela da sessão foi de **2 min 31 s** (`Done` 17:53:25 → `stop` 17:55:56), o que fica registrado porque esta folha é a evidência inteira do gate |
| `EscalaDaShellTest` | que nenhum `scale` é aplicado a um `PoseStack` dentro de `client/vfx` — a causa mais barata de a aura descolar nas articulações, e que estava escrita em quatro javadocs e cobrada em lugar nenhum. Mira o TIPO, não o nome da variável; ignora menção em comentário; permite `Vec3.scale` | **que a shell não descola.** Ele elimina UMA causa, não o sintoma: pivô de osso errado, adaptador (`HumanoidAuraAdapter`/`GeoAuraAdapter`) ou `Matrix4f` montado à mão descolam a aura com este portão verde. A pergunta continua sendo respondida por olho humano nas capturas `ten_correndo`, `ten_agachado` e `ten_nadando` do AV0. E ele lê TEXTO do pacote `client/vfx`: escala por reflexão, ou aplicada fora dali, ou dentro de um shader, não aparece |
...
| `runGameTestServer` | jogo carregado, perfil/runtime e 120 ticks de Aura pelo scheduler real; **146 cenários** (medido em `f5ae73b`), entre eles os inimigos: geometria do ponto fraco, fase de ataque voltando para IDLE, manada não se ferindo, e o agarrão do sapo montando, recusando desmontagem e soltando por tempo e por morte | rede física, desenho do cliente e performance não são provados pelo listener de captura; a vítima dos cenários de inimigo é um golem, não um jogador. E **que UMA execução verde signifique verde**: em 2026-09-21 mediu-se **1 reprovação em 9 execuções** (`1 required tests failed`), sem que o teste culpado fosse identificado em oito tentativas de reproduzir — logo uma passagem única não distingue "passou" de "passou desta vez". Pior, **sem `--rerun-tasks` a tarefa devolve cache e "passa" sem subir servidor nenhum** |
...
| O **passe de brilho rodou numa GPU em 2026-09-21**, e a primeira coisa que fez foi derrubar o cliente | a cadeia compila, tem teste da aritmética, e agora tem pixels: o passe pedia um segundo `VertexConsumer` com o primeiro ainda na mão, e o `BufferSource` encerrava o primeiro por dentro — `Not building!` ao ligar Ren com dois jogadores. Corrigido em #299, com portão estrutural, e **confirmado em jogo na mesma sessão**: os três níveis de bloom trocam, desenham e saem com sufixo próprio no nome (#300). O que continua sem olho humano é se o halo **está bonito e respeita parede** | gate do AV5 (#198) |
...
| **Água, vidro tingido e gelo não ocluem o halo — mas FOLHAS e VIDRO COMUM ocluem** | esta linha estava ERRADA até 2026-09-22 e dizia que folhas e vidro não ocluíam. Ela descrevia o estado ANTERIOR à correção da grama: a profundidade passou de `AFTER_SOLID_BLOCKS` para `AFTER_CUTOUT_BLOCKS`, e a máscara passou a conter sólido + cutout mipado + cutout. Folha é cutout mipado (ou sólido em *Fast*) e vidro comum é cutout — **os dois entraram na máscara junto com a grama**. Ficam de fora só os TRANSLÚCIDOS: água, gelo, vidro tingido e vidro colorido. Derivado do código, **não observado na tela** | confirmar de olho no gate do AV5 (#198); copiar depois dos translúcidos exigiria redesenhar a aura fora do laço de entidades |
...
| **Os contadores de VFX nunca foram transcritos — AV2 e AV3** | Nos dois gates eles foram olhados e aprovados pelo olho, e nos dois os VALORES ficaram sem registro. O AV8 mede com `spark` de qualquer jeito, entao a medicao vai acontecer; o que nao existira e a **linha de base para comparar a deriva** entre o AV2 e o AV8. Um orcamento que piorou no meio do caminho aparecera no AV8 como numero ruim, sem data de inicio | AV8 (#207), que passa a ser o primeiro numero escrito |
| **O ripple POR FAIXA nunca foi visto** | O servidor passou a dizer onde o golpe caiu (`ImpactoDeAuraS2C`), e a faixa atingida acende mais que o resto. O que existe e teste de unidade sobre a distribuicao; que a diferenca entre faixa e eco seja **perceptivel na tela** e olho, e ninguem olhou. A resposta antiga (`ripple-acende`) foi RETIRADA do registro por isso | primeira sessao com dois jogadores |
| **A tecla de foco (`G`) nunca foi apertada em jogo** | `EscolherFocoC2S` chega ao servidor, `NenGyoService.escolher` grava e o recalculo roda -- tudo coberto por teste. O que nao foi visto: a aura MUDANDO DE LUGAR ao apertar, e o aviso da action bar aparecendo. Tambem nao foi exercitado com **dois jogadores concentrando em regioes diferentes**, que e o caso que o proprio javadoc do servico cita como o erro no 2 do CLAUDE.md | primeira sessao com dois jogadores |
| **A troca de protocolo (v9 -> v10) nunca foi exercitada contra um cliente velho** | Duas linhas novas na tabela e a versao subiu. Um cliente v9 num servidor v10 deve ser recusado pelo aperto de mao; isso e construcao, e nao medicao | quando houver dois builds para cruzar |
| **Em Zetsu, o ripple nao acende -- mas o PACOTE continua saindo** | O criterio do #103 pedia que quem esta em Zetsu nao gerasse ripple visivel para observador nenhum. Na tela isso esta cumprido, e por aritmetica: a distribuicao de Zetsu e zero em toda regiao, e `comImpacto` MULTIPLICA -- zero vezes qualquer ganho continua zero. O que nao esta cumprido e o trafego: `anunciarImpacto` nao pergunta por Zetsu, e manda `ImpactoDeAuraS2C` para todo mundo que rastreia a vitima. Nao vaza aura nem dano bruto (a forca chega normalizada), mas vaza **que aquela pessoa apanhou, e em que faixa** -- de alguem cujo estado inteiro e nao ser percebido | quando In existir, que e quando decidir POR OBSERVADOR deixa de ser opcional (#162) |
| **O conserto do Ken nunca foi visto na tela** | Ken, Gyo, Shu e Ko ligadas sozinhas passaram a desenhar shell, e Gyo/Shu/Ko passaram a ser percebidas por terceiros. Tudo isso tem portao de TABELA -- completude e acordo entre observadores. **Ninguem viu a aura do Ken acender**, nem comparou dois clientes com Gyo ligado. E a mudanca tambem alterou o que os OUTROS percebem, que e comportamento de servidor: antes Ko sozinho nao produzia sinal nenhum, agora produz `REN` | gate do M4 com dois clientes (#91) |
| **A forma da aura para terceiros (v11) nunca foi vista na tela** | Gyo, Ko e Shu concentravam num ponto para o proprio jogador e acendiam o corpo inteiro para quem olhava -- relatado em jogo em 2026-09-26. A alocacao passou a viajar em `aura_presence`, quantizada. O portao prova que a forma ATRAVESSA com a regiao dominante intacta; **ninguem viu dois clientes lado a lado** confirmando que o punho de Ko acende so no punho. Tambem nao foi medido o custo do pacote a mais com varios jogadores concentrando ao mesmo tempo | gate do M4 com dois clientes (#91) |
| **Shu nao poe aura NENHUMA no item** | O relato de 2026-09-26 pede que a aura va e se acumule no item segurado. Hoje Shu concentra no braco dominante e o item nao recebe desenho nenhum -- `client/vfx` so conhece a shell do corpo. **E ausencia de implementacao, e nao de verificacao**: nao ha o que olhar | quando houver uma camada de render para o item na mao |
| ~~**Nenhum portao liga as tabelas de tecnica -> modo visual**~~ | ✅ **FECHADO em 2026-09-25.** Eram TRES tabelas, e nao duas -- a terceira era `PresencaDeAura`, no servidor. A precedencia virou uma so, em `nen/technique/PrecedenciaDeTecnicas`, e `ModoVisualConsistenteTest` e parametrizado sobre as sete tecnicas e os dois observadores. Alimentado com o Ken sem modo, com o Ko sem sinal e com uma tecnica nova, ele reprova nos tres casos nomeando a tecnica | — |
| **A HUD redesenhada nunca foi vista na tela** | O redesenho de 2026-09-25 trocou a moldura de PNG por desenho em codigo, encolheu o painel de 250x63 para 200x44, acrescentou a barra de Vida e o chip de estado, e poe o Fluxo em contexto. Tudo isso tem portao de GEOMETRIA -- alinhamento de coluna, vazamento, colapso de barra, faixa de largura. **Nada disso prova que esta bonito, nem que a escada da diagonal nao serrilha feio em GUI Scale 1, nem que o texto cabe com um nome de dezesseis caracteres.** Esses sao os criterios do proprio plano, e sao todos do olho | primeira sessao com o cliente aberto |
| **A linguagem visual "Hunter Analyzer" nunca foi vista na tela** | O redesenho de 2026-09-26 tirou a caixa: plataforma so atras da leitura, dois colchetes em cantos opostos, trilho com nos de derivacao, nucleo de retrato com aro chanfrado e oito segmentos que acendem com a aura, barras de naturezas diferentes (vital solida x fluxo segmentado com crista). **Os portoes cobram GEOMETRIA** -- o retrato fora da plataforma, alturas diferentes, celulas num numero util. Nenhum deles responde se a peca LE como instrumento, se a crista cansa em cinco minutos, ou se os oito segmentos sao legiveis em GUI Scale 1 | primeira sessao com o cliente aberto |
| **O anel de Greed Island nunca foi usado para SAIR** | Ele passou a atravessar nos dois sentidos, gravando a origem no `getPersistentData()` do jogador. O que nao foi exercitado: sair de um save ANTERIOR a esta versao (cai no spawn do Overworld, por construcao), sair depois de fechar e reabrir o jogo, e o caso de a dimensao de origem ter sumido do servidor | uma sessao com a ilha |
| **O flash de dano, o fade do chip e a rampa de Zetsu nunca foram vistos em movimento** | As tres curvas estao provadas ponto a ponto em `AnimacoesDaHudTest`, com o relogio injetado -- monotonicidade, borda, continuidade ao inverter no meio, reinicio ao trocar de mundo. O que o teste nao responde e se a DURACAO esta certa: seis ticks de flash pode ser rapido demais para o olho registrar, e dez de rampa pode ser lento demais para parecer resposta | primeira sessao com o cliente aberto |
| **As duas chaves de ocultar o vanilla nunca foram ligadas em jogo** | `OcultacaoDoVanilla` cancela a camada por evento, e a regra pura tem portao, incluindo o caso que importa: sem a HUD de Nen na tela, nada e ocultado. O que ninguem exercitou e o cancelamento DE VERDADE contra o `RenderGuiLayerEvent` do NeoForge -- se o id da camada mudar de nome numa versao futura, a regra passa a nunca casar e a opcao vira um botao morto, sem erro nenhum | uma sessao com a config ligada |
| **O nível `OFF` nunca foi olhado, e o ADR-016 promete que ele basta** | O ADR-016 escreve que com `OFF` *"a aura continua legível pela borda Fresnel e pelas ribbons"*. Em 2026-09-22, no AV3, o relato foi o oposto: com `particulas 0` e `bloom off` sobram filamentos e um resto de shell, *"bem transparentes, quase não dá para ver"*. A causa é conhecida — `FAST` compensa a falta do composite por geometria (`REFORCO_DE_HALO_EM_FAST = 1.9`, dois degraus extras) e **`OFF` não compensa com nada**, herdando alphas calibrados com o bloom ligado | gate do AV5 (#198) |
| **O registro de respostas lê arquivo, e julgamento visual não mora em arquivo** | `RespostasAindaValidasTest` invalida um veredito carregado quando os arquivos de que ele depende mudam. O que não move digest nenhum: resource pack, driver de vídeo, GPU diferente, uma mudança no próprio Minecraft — e qualquer um deles pode invalidar um julgamento visual. Os invalidadores são os que estão ao nosso alcance | uma sessão nova, quando a máquina ou o ambiente mudar |
